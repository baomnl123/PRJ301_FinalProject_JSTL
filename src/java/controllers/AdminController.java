...
public class AdminController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String totalUser = userDAO.getTotalUsers();
            String totalProduct = productDAO.getTotalProducts();
            
            if(userList.size() > 0) {
                request.setAttribute("USER_LIST", userList);
                request.setAttribute("TOTAL_USER", totalUser);
                request.setAttribute("TOTAL_PRODUCT", totalProduct);
                url = SUCCESS;
            } else {
                request.setAttribute("MESSAGE", "You don't have any users!");
            }
        } catch (Exception e) {
